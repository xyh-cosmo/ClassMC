...
struct background
{
    ...
    double Omega0_m;    /**< total sum of all non-relativistic matter. added by YHX */
...
/*=========================================================================================*/
/*  for non-parametric reconstruction of dark energy equation of state (EoS)               */
/*=========================================================================================*/
...
//  add a few extra {zi,wi} to the interpolation, but the CPZ continuity prior would not act on these {wi}
    int     DDE_table_size_extra;   // determined from DDE_z_extra in the input *ini file
    double *DDE_z_extra;    // it will be appended to *DDE_z
    double *DDE_w_extra;    // it will be appended to *DDE_w

    double DDE_EoS_at_high_z;
    double DDE_weff_at_high_z;
    double DDE_z_max;   // max(DDE_z[], DDE_z_extra[])

//  approximation option:
    int DDE_approx_opt;  // 0,2 == interpolation(s); 1 == piecewise constant

//  Option 1: use interpolation methods: linear or cubic
    //  DDE_approx_opt = 0
    gsl_interp_accel    *acc_DDE_w; //  this will always be used !!!!

#if defined(_DDE_APPROX_OPT_0_) || defined(_DDE_APPROX_OPT_2_)
    gsl_interp_accel    *acc_DDE_weff;
    gsl_spline          *spline_DDE_w;
    gsl_spline          *spline_DDE_weff;
    int     w_gsl_interp_method;  // 0: linear; 1: cubic spline. by default we will use linear spline
    int     weff_gsl_interp_method;  // 0: linear; 1: cubic spline. by default we will use cubic spline
    int     spline_DDE_weff_size;   // at least equal to DDE_table_size, could be larger to improve interpolating precision
    int     spline_is_ready;
    double  romberg_weff_eps;
#endif

//  Option 2: use piecewise constant approximation
    // DDE_approx_opt = 1
    //  by default we use scale factor bins of equal width
    
#if defined(_DDE_APPROX_OPT_1_)
    double *DDE_w_integrand;    // \int w(z)/(1+z) dz
#endif
...
int background_DDE_init2(
    struct background *pba
);

int background_DDE_init3(
    struct background *pba
);
...
#ifndef _ROMBERG_ITER_MAX_
    #define _ROMBERG_ITER_MAX_ 100
#endif
int Romberg_Integrator(
    struct background *pba,
    double xi[],
    double yi[],
    int size,
    ...
    double eps,
    double *result,
    int spline_type /* 0=linear, 1=cubic */
);
