...
/*    if (x < x_array[inf]-1e-15) { // to avoid numerical error*/
    if (x < x_array[inf]) {
      sprintf(errmsg,"%s(L:%d) : x=%e < x_min=%e",__func__,__LINE__,x,x_array[inf]);
/*      printf("@ ---> bug A:  %s(L:%d) : x=%e < x_min=%e",__func__,__LINE__,x,x_array[inf]);*/
      return _FAILURE_;
    }

    if (x > x_array[sup]+1e-10) { // to avoid numerical error
/*    if (x > x_array[sup]) {*/
      sprintf(errmsg,"%s(L:%d) : x=%e > x_max=%e",__func__,__LINE__,x,x_array[sup]);
/*      printf("@ ---> bug B:  x = %20.15f  x_inf = %20.15f  x_sup = %20.15f  x-x_sup = %20.15f\n",x,x_array[inf],x_array[sup],x-x_array[sup]);*/
/*exit(0);*/
      return _FAILURE_;
    }
...
/*    if (x < x_array[sup]-1e-15) { // to avoid numerical error*/
    if (x < x_array[sup]) {
      sprintf(errmsg,"%s(L:%d) : x=%e < x_min=%e",__func__,__LINE__,x,x_array[sup]);
      printf("@ ---> bug C: %s(L:%d) : x=%e < x_min=%e",__func__,__LINE__,x,x_array[sup]);
      return _FAILURE_;
    }
...
/*    if (x > x_array[inf]+1e-15) { // to avoid numerical error*/
    if (x > x_array[inf]) {
      ...
      printf("@ ---> bug D: %s(L:%d) : x=%e > x_max=%e",__func__,__LINE__,x,x_array[inf]);
      return _FAILURE_;
    }
