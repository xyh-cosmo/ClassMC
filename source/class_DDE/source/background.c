...
/*    printf("pba->tau_table intermode = inter_normal\n");*/
...
/*    printf("pba->tau_table intermode = inter_closeby\n");*/
...
int background_functions(
    ...
    /* model-independent w(a). added by Youhua Xu @ Jan-3, 2017 */
...
/*        printf("### debug ### --> z = %10.6f DE pressure / |p_tot| = %g\n",z,w*pvecback[pba->index_bg_rho_DDE]/fabs(p_tot));*/
    }
    ...
    if (return_format == pba->long_info) {
        ...
        pba->Omega0_m = pba->Omega0_b + pba->Omega0_cdm; // added by Youhua Xu
...
int background_free_input(
    ...
    if (pba->Omega0_DDE != 0.0 ) {
        ...
        if (pba->DDE_z_extra != NULL )
            free( pba->DDE_z_extra);
        ...
        if (pba->spline_DDE_weff != NULL)
            gsl_spline_free(pba->spline_DDE_weff);

        if (pba->acc_DDE_weff != NULL )
            gsl_interp_accel_free(pba->acc_DDE_weff);
#endif

//  pba->acc_DDE_w will always be used 
        if (pba->acc_DDE_w != NULL )
            gsl_interp_accel_free(pba->acc_DDE_w);

#if defined(_DDE_APPROX_OPT_1_)
        if (pba->DDE_w_integrand != NULL )
            free( pba->DDE_w_integrand ); 
#endif
    }
...
//  NOTE: pba->spline_DDE_w and pba->spline_DDE_weff may have different size.
    ...
    switch ( pba->w_gsl_interp_method ){
        case 0: // linear
            pba->spline_DDE_w    = gsl_spline_alloc(gsl_interp_linear,pba->DDE_table_size+pba->DDE_table_size_extra);
            break;
        case 1: // cubic
            pba->spline_DDE_w    = gsl_spline_alloc(gsl_interp_cspline,pba->DDE_table_size+pba->DDE_table_size_extra);
            break;
    }

    switch( pba->weff_gsl_interp_method ){
        case 0: // linear
            pba->spline_DDE_weff = gsl_spline_alloc(gsl_interp_linear,pba->spline_DDE_weff_size);
            break;
        case 1:  // cubic
            pba->spline_DDE_weff = gsl_spline_alloc(gsl_interp_cspline,pba->spline_DDE_weff_size);
            break;
    }
...
//	weff(z) = (ln(1+z))^-1 \int_0^z w(z')/(1+z') dz' 
    for( i=0; i<pba->DDE_table_size+pba->DDE_table_size_extra; i++ ) {
...
/*********************************/
/** added by YHX @ March-20-2019 */
/*********************************/
/*
  This second version adopts the piecewise constant approximation.
  The main difference with background_DDE_init(*) is on the initialization of w(z) and weff(z)
*/

#if defined(_DDE_APPROX_OPT_1_)
int background_DDE_init2( struct background *pba ) {

    if( pba->DDE_w_integrand != NULL ){
        free(pba->DDE_w_integrand);
    }

    class_alloc(pba->DDE_w_integrand, sizeof(double)*(pba->DDE_table_size+pba->DDE_table_size_extra), pba->error_message);
    ...
    pba->DDE_w_integrand[0] = 0;

    for( i=1; i<pba->DDE_table_size+pba->DDE_table_size_extra; i++ ){
      tmp += pba->DDE_w[i]*log((1.+pba->DDE_z[i])/(1.+pba->DDE_z[i-1]));
      pba->DDE_w_integrand[i] = tmp;
    }
...
int Romberg_Integrator( struct background *pba,
                        double xi[],
                        double yi[],
                        int size,
                        double a,
    ...
    class_test( a < xi[0], pba->error_message, "a should be >= xi[0]");
    class_test( b > xi[size-1], pba->error_message, "b should be <= xi[size-1]");
    class_test( h <= 0, pba->error_message, "stepsize h should be positive!");

    y[0] = 0.5 * h * ( gsl_spline_eval(spline, a, acc) + gsl_spline_eval(spline, b, acc) );
    m = n = 1;
    ep = eps + 1.0;
...
//  the precision requirement still has not been satisified
    class_test( (m >= _ROMBERG_ITER_MAX_) && (ep >= eps),
                pba->error_message,
                "the iteration has exceeded the maximum number, but the presion %e still has not been reached!",eps);
    *result = q;
    return _SUCCESS_;
}
...
int background_DDE_get_EoS(
    ...
    if( z >= pba->DDE_z_max ) { // when z > DDE_z_max, we assume that w is constant !!!
      *w    = pba->DDE_EoS_at_high_z;
      *weff = ( pba->DDE_weff_at_high_z*log(1.+pba->DDE_z_max) + (*w)*log( (1.+z)/(1.+pba->DDE_z_max) ) ) / log(1.+z);
    }
    else { // interpolated over pre-computed tables of w and weff
      *w    = gsl_spline_eval( pba->spline_DDE_w, z, pba->acc_DDE_w );
      *weff = gsl_spline_eval( pba->spline_DDE_weff, z, pba->acc_DDE_weff );
    }
#elif defined(_DDE_APPROX_OPT_1_)    // piecewise constant
    ...
    if( z < 1e-5 ){
        *w = pba->DDE_w[1];  // pba->DDE_w[0] is actually NOT NEEDED!!!!
        *weff = *w;
        return _SUCCESS_;
    }

    int idx=-1;
    idx = gsl_interp_accel_find(pba->acc_DDE_w, pba->DDE_z, pba->DDE_table_size+pba->DDE_table_size_extra, z);
    *w  = pba->DDE_w[idx+1];

    double tmp = pba->DDE_w_integrand[idx] + pba->DDE_w[idx+1]*log((1.+z)/(1.+pba->DDE_z[idx]));
    *weff = tmp/log(1.+z);
...
// printf("pvecback_integration[pba->index_bi_a] = %g\n", pvecback_integration[pba->index_bi_a]);
