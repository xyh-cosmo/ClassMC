...
int input_read_parameters(
    ...
    /** - Omega0_lambda (cosmological constant),
          ...
          Omega0_DDE (EoS is approximated by an interpolation on a series of {ai,wi}.  added by Youhua Xu @Jan-4-2017)
      */

    int flag_DDE;
    int use_DDE = _FALSE_;
    double param_DDE;
    class_call( parser_read_int(pfc,"use_DDE", &use_DDE, &flag_DDE, errmsg), 
				errmsg, 
				errmsg );

    /* added by Youhua Xu */
    class_call( parser_read_double(pfc,"Omega_DDE",&param_DDE,&flag_DDE,errmsg),
                errmsg,
                errmsg);
    ...
    if (use_DDE == _TRUE_) {
      ...
      // by default, we will choose to use interpolation
        pba->DDE_approx_opt = 0; 
        int DDE_approx_opt_flag;
        class_call( parser_read_int(pfc,"DDE_approx_opt",&(pba->DDE_approx_opt),&DDE_approx_opt_flag,errmsg),
            errmsg,
            errmsg );
        ...
        int z_extra_size, w_extra_size;
        int flag_z_extra, flag_w_extra;
        ...
        if( flag_z_extra == _TRUE_ ) {
            w_extra_size = z_extra_size;
            pba->DDE_table_size_extra = z_extra_size;
        }
        else {
            z_extra_size = 0;
            w_extra_size = 0;
            pba->DDE_table_size_extra = 0;
        }
        ...
        /* readl {w} from DDE_wi, where i=0,...,DDE_table_size-1. In this case
           ...
           be correctly read in, error will be return. */
            char DDE_wi[10];
            w_size = z_size;
        ...
        if( pba->DDE_table_size_extra > 0 ) {
            int j;
            char DDE_w_extra[30];
            class_alloc(array_w_extra,sizeof(double)*w_extra_size, errmsg);

            for( j=0; j<w_extra_size; j++ ) {
                sprintf(DDE_w_extra,"DDE_w_extra_%d",j);
                class_call( parser_read_double( pfc,
                                                DDE_w_extra,
                                                &(array_w_extra[j]),
                                                &DDE_w_format_flag,
        ...
        int spline_DDE_weff_size, spline_DDE_weff_size_flag;
        class_call(parser_read_int( pfc,
                                    "spline_DDE_weff_size",
                                    &spline_DDE_weff_size,
                                    &spline_DDE_weff_size_flag,
                                    errmsg),
                   errmsg,
                   errmsg);

        if( spline_DDE_weff_size_flag == _TRUE_ ) {
            pba->spline_DDE_weff_size = spline_DDE_weff_size;
        }

        // linear interpolation is the default option
...
//  the GSL interpolation (method) and Romberge integration settings are needed only when one of "_DDE_APPROX_OPT_0_" is set
        ...
        class_call(parser_read_int( pfc,
                                    "w_gsl_interp_method",
                                    &w_gsl_interp_method,
                                    &w_gsl_interp_method_flag,
                                    errmsg),
                   errmsg,
                   errmsg);

        if( w_gsl_interp_method_flag == _TRUE_ ) {
            class_test( (w_gsl_interp_method!=0) && (w_gsl_interp_method!=1),
                        errmsg,
                        "currently only 0 or 1 are acceptable for w_gsl_interp_method! (0 is recommended!!)");

            pba->w_gsl_interp_method = w_gsl_interp_method;
        }

        class_call(parser_read_int( pfc,
                                    "weff_gsl_interp_method",
                                    &weff_gsl_interp_method,
                                    &weff_gsl_interp_method_flag,
                                    errmsg),
                   errmsg,
                   errmsg);

        if( weff_gsl_interp_method_flag == _TRUE_ ) {
            class_test( (weff_gsl_interp_method!=0) && (weff_gsl_interp_method!=1),
                        errmsg,
                        "currently only 0 or 1 are acceptable for weff_gsl_interp_method!");

            pba->weff_gsl_interp_method = weff_gsl_interp_method;
        }

        int romberg_weff_eps_flag;
        double romberg_weff_eps;
        class_call( parser_read_double( pfc,
                                        "romberg_weff_eps",
                                        &romberg_weff_eps,
                                        &romberg_weff_eps_flag,
                                        errmsg),
                    errmsg,
                    errmsg );
        if( romberg_weff_eps_flag == _TRUE_ ) {
            if( romberg_weff_eps > 0. )
                pba->romberg_weff_eps = romberg_weff_eps;
        }
        else{
            pba->romberg_weff_eps = 1.e-8; // default precision level
        }
#endif

        //  array_z[] should be in increasing oder
        int DDE_z_max_flag;
        double DDE_z_max;
        pba->DDE_z_max = -1.0;
        class_call( parser_read_double( pfc,
        			...
        			errmsg,
        			errmsg );
        class_test( (DDE_z_max_flag != _TRUE_),
        			errmsg,
        			"DDE_z_max is not found!");
        			
        pba->DDE_z_max = DDE_z_max;

        class_alloc(pba->DDE_z, (pba->DDE_table_size+z_extra_size)*sizeof(double),errmsg);
        class_alloc(pba->DDE_w, (pba->DDE_table_size+w_extra_size)*sizeof(double),errmsg); 

#if defined(_DDE_APPROX_OPT_0_)
    // in this case, pba->DDE_table_size = z_size
        int i;
...
#elif defined(_DDE_APPROX_OPT_1_)
    // [[piecewise constant approximation]] in this case, pba->DDE_table_size = z_size+1
        int i;

        pba->DDE_z[0] = 0;          // pba->DDE_z[0] and pba->DE_w[0] are actually not used any more ...
        pba->DDE_w[0] = array_w[0];

        for( i=1; i<pba->DDE_table_size; i++ ) {
            pba->DDE_z[i] = array_z[i-1];
            pba->DDE_w[i] = array_w[i-1];
        ...
        if( pba->DDE_table_size_extra > 0 ) {
            ...
            class_alloc(pba->DDE_z_extra, z_extra_size*sizeof(double),errmsg);
            class_alloc(pba->DDE_w_extra, w_extra_size*sizeof(double),errmsg);

            for( i=0; i<pba->DDE_table_size_extra; i++ ) {

                // these two lines could be removd ...
                pba->DDE_z_extra[i] = array_z_extra[i];
                pba->DDE_w_extra[i] = array_w_extra[i];

                pba->DDE_z[i+pba->DDE_table_size] = array_z_extra[i];
                pba->DDE_w[i+pba->DDE_table_size] = array_w_extra[i];

                if( pba->DDE_z_max < array_z_extra[i] )
                    pba->DDE_z_max = array_z_extra[i];
            }

            if( array_z_extra != NULL )
                free(array_z_extra);
            if( array_w_extra != NULL )
                free(array_w_extra);
        }
        else {
            // if not set them to NULL pointers, memory free operation error may happen.
            pba->DDE_z_extra = NULL;
            pba->DDE_w_extra = NULL;
        }

#if defined (_DEBUG_WEFF_)
		printf("--------------------------------------------------------------------\n");
		printf("==> the following outputs should be exactly the same as your inputs!\n");
		printf("--------------------------------------------------------------------\n");
        int k;
        
        printf("==> Normal (z_i,w_i)\n");
        for( k=0; k<pba->DDE_table_size; k++) {
            printf("debug weff : z = %8.6f\tw = %10.8f\n",pba->DDE_z[k],pba->DDE_w[k]);
        }
        
        printf("==> Extra (z_i,w_i)\n");
        for( k=0; k<pba->DDE_table_size_extra; k++) {
            printf("debug weff : z = %8.6f\tw = %10.8f\n",pba->DDE_z[pba->DDE_table_size+k],pba->DDE_w[pba->DDE_table_size+k]);
        }
...
/*        exit(0);*/
...
#if defined(_DDE_APPROX_OPT_0_)
        class_call( background_DDE_init(pba), 
                  pba->error_message, 
                  errmsg);
#elif defined(_DDE_APPROX_OPT_1_)
        pba->DDE_w_integrand = NULL;
        ...
        class_test((flag1 == _TRUE_) && (flag2 == _TRUE_) && ((flag3 == _FALSE_) || (param3 >= 0.)),
                   errmsg,
                   "In input file, either Omega_Lambda or Omega_fld must be left unspecified, except \
                   if Omega_scf is set and <0.0, in which case the contribution from the scalar field \
                   will be the free parameter.");
...
/*
    else{ // added by YOUHUA XU for debug...
    	printf("did not find reio_parametrization !!!\n");
    	exit(0);
    }
*/
...
int input_default_params(
    ...
    pba->Omega0_m = pba->Omega0_b+pba->Omega0_cdm;  // added by Youhua Xu @ Jul-6,2018
    ...
    ppt->l_lss_max=300;
    ppt->k_max_for_pk=2;  // chenged 0.1 to 2.0, this will assure the accuracy of the evalution of sigma8 
